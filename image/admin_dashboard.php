...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['add_grade'])) {
        ...
        $period = $_POST['period'];
        $quizzes = validate_grade($_POST['quizzes']);
        $academic_projects = validate_grade($_POST['academic_projects']);
        $recitation = validate_grade($_POST['recitation']);
        $performance_task = validate_grade($_POST['performance_task']);
        $activity_sheets = validate_grade($_POST['activity_sheets']);
        $assignments = validate_grade($_POST['assignments']);
        ...
        if ($period == 'PE') {
            $prelim_exam = validate_grade($_POST['prelim_exam']);
        } elseif ($period == 'ME') {
            $midterm_exam = validate_grade($_POST['midterm_exam']);
        } elseif ($period == 'SFE') {
            $semi_final_exam = validate_grade($_POST['semi_final_exam']);
        } elseif ($period == 'FE') {
            $final_exam = validate_grade($_POST['final_exam']);
        }
...
function determine_remarks($final_grade_average) {
    return $final_grade_average < 74 ? 'Failed' : 'Passed';
}
?>
<!DOCTYPE html>
<html lang="en">
